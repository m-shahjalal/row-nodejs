...
handler._user.post = (request, callback) => {
  const firstName = typeof request.body.firstName === 'string' && request.body.firstName.trim().length > 0
      ? request.body.firstName
      : false;

  const lastName = typeof request.body.lastName === 'string' && request.body.lastName.trim().length > 0
  ...
  const tosAgreement = typeof request.body.tosAgreement === 'boolean' ? request.body.tosAgreement : false;
  ...
  if (firstName && lastName && phone && password && tosAgreement) {
    ...
    callback(400, { error: 'There is a problem, check the form and try again' });
  }
};

handler._user.get = (request, callback) => {
  const phone =
    typeof request.queryObject.phone === 'string' && request.queryObject.phone.trim().length === 11
      ...
      data.read('users', phone, (err, d) => {
        ...
        if (!err && user) {
          ...
          data.update('users', phone, user, (err2) => {
            if (err2) {
              callback(201, { message: 'Update data successfully'})
            } else {
              callback(400, {message: 'Update data failed'})
            }
          })
        }
      })
...
handler._user.delete = (request, callback) => {
    const phone =    typeof request.queryObject.phone === 'string' && request.queryObject.phone.trim().length === 11
      ? request.queryObject.phone
      : false;
    ...
    data.read('users', phone, (error, userData) => {
      if (!error && userData) {
        data.delete('users', phone, (err) => {
          if (err) {
            callback(200, { message: "data deleted successfully"})
          } else {
